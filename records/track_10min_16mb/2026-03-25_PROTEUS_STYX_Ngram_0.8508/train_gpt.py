...
class Hyperparameters:
    ...
    styx_gate = bool(int(os.environ.get("STYX_GATE", "0")))
...
class StyxGate(nn.Module):
    """PROTEUS+STYX: Per-token content importance gate.
    Learns to identify binding tokens (high importance) vs noise tokens (low importance).
    Produces a per-token scalar that modulates attention output before residual addition.
    STYX principle: spend capacity on what matters, attenuate what doesn't."""
    def __init__(self, dim: int):
        super().__init__()
        self.proj = nn.Linear(dim, 1, bias=True)
        nn.init.zeros_(self.proj.weight)
        nn.init.constant_(self.proj.bias, 2.0)  # sigmoid(2)≈0.88, starts near pass-through

    def forward(self, x: Tensor) -> Tensor:
        """x: (B, T, D) -> importance: (B, T, 1) in [0, 1]"""
        return torch.sigmoid(self.proj(x.detach()))  # detach: gate learns from loss, not from input grad
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, styx_importance: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, raw_v = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        if styx_importance is not None:
            attn_out = attn_out * styx_importance  # STYX: modulate attention by token importance
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        mlp_out = self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor, up_w, down_w)
        if styx_importance is not None:
            mlp_out = mlp_out * styx_importance  # STYX: modulate MLP by token importance
...
class GPT(nn.Module):
    def __init__(
        ...
        self.styx_gate = StyxGate(model_dim) if styx_gate else None
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self.smear(x)
        x0 = x
        styx_imp = self.styx_gate(x0) if self.styx_gate is not None else None  # (B, T, 1)
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0, styx_importance=styx_imp)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        x = self.smear(x)
        x0 = x
        styx_imp = self.styx_gate(x0) if self.styx_gate is not None else None
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        for i in range(self.num_encoder_layers):
...
# --- Fast N-gram eval cache (numpy vectorized, no Python loops) ---

class FastNgramCache:
    """Multi-order n-gram cache with backoff (5→4→3→2).
    For each token, tries longest matching context first.
    Falls back to shorter contexts if no match at higher order.
    Bigrams always have hits. Vectorized numpy, no Python per-token loops.
    Legal: backward-looking, fixed alpha, no oracle gate.
    Memory: 4 orders × 2 tables × num_buckets × 4 bytes = ~128MB at 4M buckets."""

    def __init__(self, vocab_size: int, max_order: int = 5, num_buckets: int = 4_194_304):
        self.max_order = max_order
        self.min_order = 2  # bigrams minimum
        self.vocab_size = vocab_size
        self.num_buckets = num_buckets
        # One pair of tables per order (2-gram through max_order-gram)
    ...
    def _hash_contexts(self, tokens: np.ndarray, ctx_len: int) -> np.ndarray:
        """Hash context windows of length ctx_len."""
        n = len(tokens)
        if n <= ctx_len:
    ...
    def update_batch(self, tokens: np.ndarray) -> None:
        """Update all orders with all n-grams in a token sequence."""
    ...
    def get_best_probs(self, tokens: np.ndarray, min_count: int = 2) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Get best available cached probability for each position using backoff.
        Tries max_order first, falls back to lower orders.
        Returns: (hit_counts, ctx_totals, best_order) for the longest context match.
        Arrays cover positions max_order-1..N-1 (aligned to max context)."""
        max_ctx = self.max_order - 1
        n = len(tokens)
        if n <= max_ctx:
            empty = np.array([], dtype=np.int64)
            return empty, empty, empty
        num_pos = n - max_ctx  # align to longest context
        ...
        for order in range(self.max_order, self.min_order - 1, -1):  # 5, 4, 3, 2
            ctx_len = order - 1
            ctx_h = self._hash_contexts(tokens, ctx_len)
            targets = tokens[ctx_len:]
            ngram_h = self._hash_ngrams(ctx_h, targets)
            ...
            # Align to max_ctx positions (higher orders produce fewer positions)
            offset = max_ctx - ctx_len  # how many positions to skip at start
...
def eval_val_sliding(
    ...
    """Sliding window evaluation with optional fast n-gram cache blending."""
    seq_len = eval_seq_len or args.train_seq_len
        ...
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                ...
                if use_cache and scored_len > 0:
                    # Get model log-probs for scored positions
                    scored_logits = logits[i, s:wlen].float()  # (scored_len, vocab)
                    scored_targets = y_batch[i, s:wlen].cpu().numpy()  # (scored_len,)
                    log_probs = F.log_softmax(scored_logits, dim=-1)  # (scored_len, vocab)
                    model_nll = -log_probs[torch.arange(scored_len), y_batch[i, s:wlen]].to(torch.float64)
                    # Multi-order n-gram cache lookup with backoff (5→4→3→2)
                    abs_start = ws + s
                    abs_end = ws + wlen
                    ...
                    # Align: get_best_probs returns positions for max_ctx..len(span)-1
                    # We need positions aligned to abs_start..abs_end-1
                    ...
                    # Update cache with scored tokens (backward-looking)
                    cache.update_batch(val_np[abs_start:abs_end + 1])
                else:
                    # Standard scoring without cache
...
def main() -> None:
    ...
    if base_model.styx_gate is not None:
        scalar_params.append(base_model.styx_gate.proj.weight)
        scalar_params.append(base_model.styx_gate.proj.bias)
