...
Status definitions:
  ...
  ALIVE      — OPEN, has BPB, passes compliance checks, not closed by maintainer.

Usage:
    python scripts/classify.py

Input:  data/pr_cache.json
Output: data/pr_cache.json  (status/flags/confidence fields added in-place)
"""
...
def classify_pr(pr: dict[str, Any]) -> dict[str, Any]:
    """Classify a single PR and return a copy with status/flags/confidence added."""
    result = dict(pr)
    state = pr.get("state", "OPEN")
...
def main() -> None:
    ...
    print(f"[DONE] Classification complete:", flush=True)
